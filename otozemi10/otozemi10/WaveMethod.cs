...
namespace Wave
{
    ...
    public static class FFT
    {
        ...
        public static void FFT_HT(double[] inputRe, double[] inputIm, out double[] outputRe, out double[] outputIm, int bitSize)
        {
            ...
            //先生のＦＦＴ
            //output = Nm.DiscreteFourierTransform(input,false);
    ...
    sealed public class WaveMethod
    {
        ...
        #region 出力WAVEファイルの波形をうちこむdumpDataメソッド
        ...
        public static double[] ReadWavFile(string fileName)//モノラル前提
        {
        ...
        #region ハミング窓
        private static double HammingWindow(int i, int N)
        {
        ...
        public static complex[][] STFT_Hanning(double[] data,int bitSize)
        {
            int dataSize = (int)Math.Pow(2, bitSize);
            double[] input = new double[dataSize];
            List<complex[]> histFFT = new List<complex[]>();
            for (int i = 0; i < 2 * (data.Length / dataSize - 1.0); i++)
            {
                for (int j = 0; j < dataSize; j++)
                {
                ...
                complex[] x = Nm.RealDiscreteFourierTransform(input);
                histFFT.Add(x);
            }
            return histFFT.ToArray();
        }
        public static complex[][] STFT_Hamming(double[] data, int bitSize)
        {
            int dataSize = (int)Math.Pow(2, bitSize);
            double[] input = new double[dataSize];
            List<complex[]> histFFT = new List<complex[]>();
            for (int i = 0; i < 2 * (data.Length / dataSize - 1.0); i++)
            {
                for (int j = 0; j < dataSize; j++)
                {
                    input[j] = data[i * dataSize / 2 + j] * HammingWindow(j, dataSize);
                }
                complex[] x = Nm.RealDiscreteFourierTransform(input);
                histFFT.Add(x);
            }
        ...
        #region 逆フーリエ変換
        public static double[][] Inv_STFT_Hamming(complex[][] data,int bitSize)
        {
            int dataSize = (int)Math.Pow(2, bitSize);
            List<double[]> histFFT = new List<double[]>();
            for (int i = 0; i < data.Count(); i++)
            {
                double[] output = Nm.RealInverseDiscreteFourierTransform(data[i]);
                for (int j = 0; j < dataSize; j++)
                {
                    output[j] /= HammingWindow(j, dataSize);
                }
                histFFT.Add(output);
            }
            return histFFT.ToArray();
        ...
        public static double[][] Inv_STFT_Hanning(complex[][] data, int bitSize)
        {
            int dataSize = (int)Math.Pow(2, bitSize);
            List<double[]> histFFT = new List<double[]>();
            for (int i = 0; i < data.Count(); i++)
            {
                ...
                for (int j = 0; j < dataSize; j++)
                {
                    output[j] /= HanningWindow(j, dataSize);
                }
                histFFT.Add(output);
            }
            return histFFT.ToArray();
        ...
        #region パワースペクトル
        public static double[][] STPS_Hamming(double[] data,int bitSize)
        {
            int dataSize = (int)Math.Pow(2, bitSize);
            complex[][] input = new complex[data.Count() / dataSize][];
            for(int i=0;i<data.Count() / (int)Math.Pow(2, bitSize);i++)
            {
                input[i] = new complex[(int)Math.Pow(2, bitSize)];
                for(int j=0;j<dataSize;j++)
                {
                    input[i][j].Re = data[i * dataSize + j] * HammingWindow(j, dataSize);
                }
            }
            ...
            for (int i = 0; i < data.Count() / (int)Math.Pow(2, bitSize); i++)
            {
                x = Nm.PowerSpectrumFFT(input[i]);
                result.Add(x);
            }
            return result.ToArray();
        }

        public static double[][] STPS_Hanning(double[] data, int bitSize)
        {
            int dataSize = (int)Math.Pow(2, bitSize);
            complex[][] input = new complex[data.Count() / dataSize][];
            for (int i = 0; i < data.Count() / (int)Math.Pow(2, bitSize); i++)
            {
                ...
                for (int j = 0; j < dataSize; j++)
                {
                    input[i][j].Re = data[i * dataSize + j] * HanningWindow(j, dataSize);
                }
            }
            List<double[]> result = new List<double[]>();
            double[] x = new double[dataSize / 2];
